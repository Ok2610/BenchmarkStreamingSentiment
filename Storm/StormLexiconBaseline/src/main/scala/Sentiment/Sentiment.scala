...
object Sentiment {
  def main(args: Array[String]): Unit = {
    ...
    val path = "hdfs://" + config.getString("cluster.hdfs.host") + config.getString("cluster.hdfs.lexiconPath")
    ...
    builder.setBolt(sentimentBoltId, new LexiconBolt(benchmarkId, path), sentimentThreads).shuffleGrouping(kafkaSpoutId)
