...
object Main {
  ...
  val kafkaMessage = List.fill(1024)("1").mkString
  ...
  def main(args: Array[String]):Unit = {
    if(args.length < 9) {
      println("Missing arguments. Example run: \njava -jar benchdriver.jar [platform] [jarFile] [messagesPerWave] [numWaves] [numProducers] [numConsumers] [kafkaReplicationFactor] [kafkaPartitions] [description@with@spaces] [waveInterval]")
      return
    }
    ...
    val kafkaReplicationFactor = args(6).toInt
    val kafkaPartitions = args(7).toInt
    val benchmarkDescription = args(8).replaceAll("@", " ") // Short text@with@spaces describing the benchmark
    val waveInterval = args(9).toInt
    ...
    if (platform == "kafka") {
      driverProducerTopic = "Consumer" + benchmarkId
    ...
    val warmUpFutures = runBenchmark(numProducers, numConsumers, platform, benchmarkId, sendRatePerWave, numWaves, waveInterval, true)
    val warmUpResults = Await.result(warmUpFutures, Duration.Inf)
    println("Warm up done")

    val consumerFutures = runBenchmark(numProducers, numConsumers, platform, benchmarkId, sendRatePerWave, numWaves, waveInterval, false)
  ...
  def startConsumers(platform: String, consumers: Array[KafkaConsumer[String,String]], topic: String, benchmarkId: String, sendRatePerWave: Int, numWaves: Int, waveInterval: Int, warmUp: Boolean): Future[Seq[List[DelayStep]]] = {
      ...
      if (!platform.equals("kafka")) {
        ...
        breakable {
          while (true) {
            val records = consumer.poll(100).asScala
            for (record <- records) {
              val wave = record.value().substring(0, record.value().indexOf(";"))
              val benchTimestamp = record.value().substring(wave.length+1, record.value().indexOf(","))
              val message = record.value().substring(benchTimestamp.length + 1)
              val currentTimestamp = Instant.now.toEpochMilli
              lastConsumedTimestamp = currentTimestamp

              if (!warmUp) {                                                                                                     
                val delay = currentTimestamp - Long.parseLong(benchTimestamp)
                futureDelayList = saveDelay(futureDelayList, benchmarkId, Long.parseLong(benchTimestamp), currentTimestamp, delay, wave, "0", "0")
                consumeCount = consumeCount + 1
              }
            }
            if (Instant.now.toEpochMilli - lastConsumedTimestamp > 30000+waveInterval) {
  ...
  def producerRunner(topic: String, producer: KafkaProducer[String, String], sendRate: Int, numWaves: Int, waveInterval: Int): Thread = {
    new Thread(() => { //Don't change to anon function
      ...
      while (waveCount < numWaves) {
        for (i <- 1 to sendRate) {
          val data = new ProducerRecord[String, String](topic, waveCount + ";" + String.valueOf(Instant.now.toEpochMilli) + "," + kafkaMessage)
          producer.send(data)
        }
  ...
  def runBenchmark(numProducers:Int, numConsumers:Int, platform:String, benchmarkId:String, sendRatePerWave:Int, numWaves:Int, waveInterval:Int, warmUp: Boolean): Future[Seq[List[DelayStep]]] = {
    ...
    if (warmUp) {
      val consumerFutures = startConsumers(platform, consumerArray, driverConsumerTopic, benchmarkId, sendRatePerWave, 1, 300000, true)
      for (i <- 0 until numProducers) {
      ...
      val consumerFutures = startConsumers(platform, consumerArray, driverConsumerTopic, benchmarkId, sendRatePerWave, numWaves, waveInterval, false)    
  ...
  def startFlink(parameters: String) {
    val flinkThread = new Thread(() => {
      (config.getString("cluster.flink.path") + "flink run -m yarn-cluster -yn " + config.getString("cluster.flink.yarn.numberOfTaskManagers") + " -p " + config.getString("cluster.flink.yarn.slots") + " ./jars/"+ jarFile + parameters).!
    })
    flinkThread.start
  }
  ...
  def saveDelay(delayList: List[DelayStep], benchmarkId: String, start:Long, end:Long, delay: Long, wave: String, frameworkStart: String, frameworkEnd: String): List[DelayStep] = {
    new DelayStep(benchmarkId, start, end, delay, wave, frameworkStart, frameworkEnd)::delayList
  }
  ...
  def setupNewBenchmark(): String = {
    var id = 0
    val idList = Source.fromFile("id.txt").getLines.toList //Source.fromInputStream(getClass.getResourceAsStream("/id.txt")).getLines.toList
