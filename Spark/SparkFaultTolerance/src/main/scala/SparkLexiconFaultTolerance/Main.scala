package SparkLexiconFaultTolerance
...
object Main {
  ...
  def functionToCreateContext(spark: SparkSession): StreamingContext = {
    val ssc = new StreamingContext(spark.sparkContext, Milliseconds(config.getLong("cluster.spark.batchInterval")))   // new context

    ssc.checkpoint("spark_checkpoints")   // set checkpoint directory
    ssc
  }
  ...
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().setAppName("SparkLexiconFaultTolerance")
    val spark = SparkSession.builder()
                            .config(sparkConf)
                            .getOrCreate()
    val ssc = StreamingContext.getOrCreate("spark_checkpoints", () => functionToCreateContext(spark))//new StreamingContext(spark.sparkContext, Milliseconds(config.getLong("cluster.spark.batchInterval")))
